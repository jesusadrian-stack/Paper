<?php
$pageTitle = 'Fondo de Corresponsal Bancario';
...
<div class="card shadow-sm border-0 rounded-4 p-4 mb-4">
    <div class="d-flex flex-column flex-sm-row justify-content-between align-items-sm-center gap-3">
        <div class="d-flex align-items-center gap-3">
            <div class="stat-icon info fs-3">
                <i class="bi bi-bank2"></i>
            </div>
            <div>
                <small class="text-muted text-uppercase fw-semibold">Fondo Operativo</small>
                <h4 class="fw-bold mb-0 text-dark"><?= sanitize($cuenta['nombre']) ?></h4>
            </div>
        </div>
        <div class="text-sm-end">
            <small class="text-muted d-block">Saldo de Reserva Bancaria</small>
            <h2 class="fw-bold text-primary mb-0"><?= formatMoney($cuenta['saldo']) ?></h2>
        </div>
    </div>
</div>
...
<div class="card shadow-sm border-0 rounded-4 p-4">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h5 class="fw-bold mb-0 text-dark">Movimientos y Flujo de Corresponsal</h5>
        <div class="d-flex gap-2">
            ...
            <a href="<?= url('transferencias/create') ?>" class="btn btn-outline-secondary btn-sm">Transferir</a>
        </div>
    </div>
        ...
        <table class="table table-custom mb-0">
            ...
            <tbody>
                <?php if (empty($movimientos)): ?>
                    <tr><td colspan="7" class="text-center text-muted py-4">No hay movimientos registrados en la cuenta de corresponsal.</td></tr>
                <?php else: ?>
